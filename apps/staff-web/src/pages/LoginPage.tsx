  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!STAFF_ROLES.includes(data.user.role)) {
        setError('Этот вход только для сотрудников автосервиса');
        return;
      }
  ...
  return (
    <div className={s.wrap}>
      <form className={s.form} onSubmit={submit}>
        <h2 className={s.title}>ВХОД ДЛЯ СОТРУДНИКОВ</h2>
        <p className={s.sub}>Доступ выдаёт владелец автосервиса</p>
